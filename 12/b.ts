...
const startingPositions: { x: number; y: number }[] = [];
...
function breadthFirstSearch(x: number, y: number) {
  ...
  while (queue.length > 0) {
    ...
    if (current) {
      ...
      if (x === finishPosition.x && y === finishPosition.y && moves > 0) {
        foundInMoves = moves;
        break;
      }
  ...
  return foundInMoves > 0 ? foundInMoves : undefined;
}

const moves = startingPositions
  .map((position) => breadthFirstSearch(position.x, position.y))
  .filter(Boolean)
  .sort((a, b) => a! - b!);
